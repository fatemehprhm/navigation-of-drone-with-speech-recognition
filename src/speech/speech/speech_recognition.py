...
def reset_audio_sample_rate(filename, dst_sample_rate):
    ...
    if (dst_sample_rate is not None) and (dst_sample_rate != sample_rate)\
        and (sample_rate != 0):
        data = librosa.core.resample(data, orig_sr=sample_rate, target_sr=16000)
...
class AudioRecorder(object):
    ...
    def init_settings(self):
        ...
        self.parser.add_argument(
            '-d', '--device', type=self.int_or_str, default='USB Camera-B4.09.24.1',
...
class SpeechRecognitionNode(Node):
    def __init__(self):
        ...
        self.board = KeyboardMonitor(self.recording_state, PRINT=False)
        self.board.start_listen(run_in_new_thread=True)

        # Set up audio recorder
        self.recorder = AudioRecorder()

        # Others
        self.tprinter = TimerPrinter() # for print
    ...
    def timer_callback(self):
        ...
        # Start loop
        self.tprinter.print("Usage: keep pressing down 'R' to record audio", T_gap=20)

        while True:
            self.board.update_key_state()
            if self.board.get_key_state()==True:
                break

        if self.board.has_just_pressed():

            self.j += 1
            print("Record {}th voice".format(self.j))
            
            # start recording
            self.recorder.start_record(folder=SAVE_AUDIO_TO) 

            # wait until key release
            while not self.board.has_just_released():
                self.board.update_key_state()
                time.sleep(0.001)
            ...
            # Do inference
            audio = self.recorder.filename
            predicted_label = self.model.transcribe(audio, fp16=False, language='English')
                
            print("\nPredicted label: {}\n".format(predicted_label["text"]))

            text = predicted_label["text"]
            msg = String()
            msg.data = text
...
def main(args=None):
    ...
    node_publisher = SpeechRecognitionNode()
